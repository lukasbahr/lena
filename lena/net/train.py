from lena.net.autoencoder import Autoencoder, NN
import torch
...
def train_nn(data, observer, params):
    ...
    for epoch in range(params['epochs']):
        ...
        for i, batch in enumerate(trainloader, 0):
            ...
            z = batch[:, dim_x:].to(device)
            x = batch[:, :dim_x].to(device)
...
def train(data, observer, params):
    ...
    # Create trainloader
    trainloader = utils.data.DataLoader(data[:, :dim_x+optionalDim], batch_size=params['batch_size'],
                                        shuffle=params['shuffle'], num_workers=2, drop_last=True)
